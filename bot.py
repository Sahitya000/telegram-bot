...
def handle_applist(message):
    ...
    # ✅ Extract Base Name Without Variants (Case-Insensitive)
    unique_apps = {}
    for full_name, apk_link in apk_links.items():
        base_name = re.split(r'[\s\-_\.]+', full_name, maxsplit=1)[0].lower()  # First word (lowercase)
        base_name = base_name.capitalize()  # First letter capital, rest small (YouTube, Snapchat)
